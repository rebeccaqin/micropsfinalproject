...
USART1_TypeDef * id2Port(uint32_t USART_ID){
    USART1_TypeDef * USART;
    switch(USART_ID){
        case(USART1_ID) :
            USART = USART11;
            break;
...
USART1_TypeDef * initUSART(uint8_t USART_ID, uint32_t baud_rate){
    RCC1->AHB1ENR.GPIOAEN = 1; // Enable GPIO port A

    USART1_TypeDef * USART = id2Port(USART_ID);

    switch(USART_ID){
        case USART1_ID :
            RCC1->APB2ENR |= (1 << 4); // Set USART1EN

            // Configure pin modes as ALT function
            pinMode(GPIOA1, GPIO_PA9, GPIO_ALT); // TX
            pinMode(GPIOA1, GPIO_PA10, GPIO_ALT); // RX

            GPIOA1->AFRH |= (0b111 << 4*2) | (0b111 << 4*1);
            break;
        case USART2_ID :
            RCC1->APB1ENR |= (1 << 17); // Set USART1EN

            // Configure pin modes as ALT function
            pinMode(GPIOA1, GPIO_PA2, GPIO_ALT); // TX
            pinMode(GPIOA1, GPIO_PA3, GPIO_ALT); // RX

            // Configure correct alternate functions (AF07)
            GPIOA1->AFRL |= (0b111 << 4*3) | (0b111 << 4*2);
            break;
    }
...
void sendChar(USART1_TypeDef * USART, uint8_t data){
    while(!USART->SR.TXE);
    USART->DR.DR = data;
    while(!USART->SR.TC);
}

void sendString(USART1_TypeDef * USART, uint8_t * charArray){
...
uint8_t readChar(USART1_TypeDef * USART){
...
void readString(USART1_TypeDef * USART, uint8_t * charArray){
    uint32_t i = 0;
    do{
